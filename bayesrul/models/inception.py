...
class InceptionModule(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(
                n_features, filter1x1, kernel_size=1, padding="same", bias=bias
            ),
            activation(),
        )
        self.conv3 = nn.Sequential(
            nn.Conv1d(
                n_features, filter3x3, kernel_size=3, padding="same", bias=bias
            ),
            activation(),
        )
        self.conv5 = nn.Sequential(
            nn.Conv1d(
                n_features, filter5x5, kernel_size=5, padding="same", bias=bias
            ),
            activation(),
        )
        self.convpool = nn.Sequential(
            ...
            nn.Conv1d(
                n_features, filterpool, kernel_size=3, padding="same", bias=bias
            ),
            activation(),
        )
        if dropout > 0:
...
class InceptionModuleReducDim(nn.Module):
    def __init__(
        ...
        self.branch2 = nn.Sequential(
            nn.Conv1d(
                n_features, reduc3x3, kernel_size=1, padding=0, bias=bias
            ),
            activation(),
            nn.Conv1d(
                reduc3x3, filter3x3, kernel_size=3, padding="same", bias=bias
            ),
            activation(),
        )
        self.branch3 = nn.Sequential(
            nn.Conv1d(
                n_features, reduc5x5, kernel_size=1, padding=0, bias=bias
            ),
            activation(),
            nn.Conv1d(
                reduc5x5, filter5x5, kernel_size=5, padding="same", bias=bias
            ),
            activation(),
        )
        self.branch4 = nn.Sequential(
            nn.MaxPool1d(kernel_size=3, stride=1, padding=1),
            nn.Conv1d(
                n_features, filterpool, kernel_size=1, padding=0, bias=bias
            ),
            activation(),
        )
        if dropout > 0:
