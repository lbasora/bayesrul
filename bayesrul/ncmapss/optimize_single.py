from bayesrul.inference import vi_bnn, dnn, mc_dropout, deep_ensemble
...
def mfvi_objective(trial: optuna.trial.Trial) -> float:
    ...
    args.archi = trial.suggest_categorical(
        "args.archi", ["linear", "conv", "inception", "bigception"]
    )
    args.activation = trial.suggest_categorical(
        "args.activation", ["leaky_relu", "tanh", "relu"]
    )
    ...
    res = inference.fit(EPOCHS, monitor=monitor)
...
def lrt_objective(trial: optuna.trial.Trial) -> float:
    ...
    optimizer = "adam"
    args.archi = trial.suggest_categorical(
        "args.archi", ["inception", "bigception"]
    )
...
def mcdropout_objective(trial: optuna.trial.Trial) -> float:
    ...
    monitor = "mse/val"
    inference = mc_dropout.MCDropout(
        args, data, p_dropout, hyperparams=hyperparams, GPU=GPU, studying=True
    )
    res = inference.fit(EPOCHS, monitor=monitor)
...
def deepensemble_objective(trial: optuna.trial.Trial) -> float:
    ...
    monitor = "mse/val"
    inference = deep_ensemble.DeepEnsemble(
        args, data, n_models, hyperparams=hyperparams, GPU=GPU, studying=True
    )
    res = inference.fit(EPOCHS, monitor=monitor)
...
def heteroscedasticnn_objective(trial: optuna.trial.Trial) -> float:
    lr = trial.suggest_float("lr", 1e-4, 1e-1, log=True)
    args.archi = trial.suggest_categorical(
        "args.archi", ["inception", "bigception", "linear", "conv"]
    )
    args.activation = trial.suggest_categorical(
        "args.activation", ["leaky_relu", "tanh", "relu"]
    )
    ...
    monitor = "mse/val"
    inference = dnn.HeteroscedasticDNN(
        args, data, hyperparams=hyperparams, GPU=GPU, studying=True
    )
    res = inference.fit(EPOCHS, monitor=monitor)
...
if __name__ == "__main__":
    ...
    if args.sampler.upper() == "RANDOM":
        ...
        raise ValueError(
            f"Unknow sampler {args.sampler}. Choose RANDOM or NSGAII"
        )

    pruner = optuna.pruners.PatientPruner(
        optuna.pruners.MedianPruner(), patience=10
    )
