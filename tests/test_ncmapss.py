...
# from bayesrul.ncmapss.preprocessing import process_parquet_files
...
def test_linear_piece_wise_RUL():
    ...
    assert all(
        new_df["rul"]
        == [6.0] * 5 + list(range(5, 0, -1)) + [8.0] * 3 + list(range(7, 0, -1))
    )
...
#     for line in process_parquet_files(pq_path.iterdir(), args):
#         s = pd.DataFrame(
#             [
#                 [
#                     line.ds_id,
...
#     assert (
#         np.linalg.norm(
#             results.loc[0].values
#             - np.array([2, 2.0, 0, 70.64600128155931, 53.0])
#         )
#         <= 1e-5
#     )
#     assert (
#         np.linalg.norm(
#             results.loc[results.shape[0] - 1].values
#             - np.array([2, 20.0, 6, 297.820554, 0])
#         )
#         <= 1e-5
#     )
